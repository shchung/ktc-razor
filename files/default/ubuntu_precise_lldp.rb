...
module ProjectRazor
  module ModelTemplate
    # Root Model object
    # @abstract
    class UbuntuPreciseLLDP < Ubuntu
      ...
      def initialize(hash)
        ...
        # Static config
        @hidden          = false
        @name            = "ubuntu_precise_lldp"
        @description     = "Ubuntu Precise Model with lldp"
        # Metadata vars
        @hostname_prefix = nil
...
#                                            :description => "MGMT dns-search" },
      ...
      def mgmt_nic
		...
		def mac_nic(nic) 
			for i in 1..@node.attributes_hash["mk_hw_nic_count"].to_i
				if (nic == 'eth0') && (@node.attributes_hash["lldp_neighbor_sysname_eth"+i.to_s].split( /(?:\s*(?:_|-)\s*)+/)[3] == 'MG')&&(@node.attributes_hash["lldp_neighbor_portid_eth"+i.to_s][-3,3].gsub(/[^0-9]/,"").to_i <18)
					then return @node.attributes_hash["macaddress_eth"+(i-1).to_s]
				elsif (nic == 'eth1') &&  (@node.attributes_hash["lldp_neighbor_sysname_eth"+i.to_s].split( /(?:\s*(?:_|-)\s*)+/)[3] != 'MG')&&(@node.attributes_hash["lldp_neighbor_portid_eth"+i.to_s][-3,3].gsub(/[^0-9]/,"").to_i <18)
					then return @node.attributes_hash["macaddress_eth"+(i-1).to_s]
				elsif (nic == 'eth2') &&  (@node.attributes_hash["lldp_neighbor_sysname_eth"+i.to_s].split( /(?:\s*(?:_|-)\s*)+/)[3] != 'MG')&&(@node.attributes_hash["lldp_neighbor_portid_eth"+i.to_s][-3,3].gsub(/[^0-9]/,"").to_i >=18)
					then return @node.attributes_hash["macaddress_eth"+(i-1).to_s]
				elsif (nic == 'eth3') &&  (@node.attributes_hash["lldp_neighbor_sysname_eth"+i.to_s] == nil) 
					then return @node.attributes_hash["macaddress_eth"+(i-1).to_s]
				end
 			return 'error' 
